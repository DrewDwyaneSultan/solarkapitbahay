...
import { validateSettingsForm } from '../utils/userMessages';

const DEFAULT_BROKER = '10.211.242.244';

function applyBarangayToForm(bg, setters, fallbackEmail = '') {
  if (!bg) return;
  const {
    setBarangayName,
    setContactEmail,
  ...
  setContactEmail(bg.contact_email?.trim() || fallbackEmail || '');
...
export default function SettingsPage({
  ...
  const [contactEmail, setContactEmail] = useState(operatorEmail ?? '');
  ...
  const [barangayLoading, setBarangayLoading] = useState(Boolean(accessToken));
    ...
    async function loadBarangay() {
      ...
      try {
        ...
        if (!formDirtyRef.current) {
          applyBarangayToForm(bg, setters, operatorEmail);
        } else if (!contactEmail && (bg.contact_email || operatorEmail)) {
          setContactEmail(bg.contact_email?.trim() || operatorEmail);
        }
        if (!formDirtyRef.current && !bg.barangay_code && headerCode) {
          setBarangayCode(headerCode);
        }
        loadedRef.current = true;
      } catch (err) {
        if (!cancelled) {
          const msg = err.message ?? 'Could not load barangay settings.';
          setLoadError(msg);
          showError(err, msg);
          if (!formDirtyRef.current && operatorEmail) {
            setContactEmail(operatorEmail);
          }
          if (headerCode) setBarangayCode(headerCode);
          if (headerName) setBarangayName(headerName);
        }
      } finally {
  ...
  }, [accessToken, operatorEmail, headerCode, headerName]);
  ...
  const doSave = async () => {
    ...
    if (!accessToken) {
      const payload = {
        ...
        message: 'You must be signed in to save barangay settings.',
        hint: 'Sign out and sign in again, then return to Settings.',
      };
      setSaveError(payload.message);
      showToast({ tone: 'error', ...payload });
      return;
    }
    ...
    try {
      ...
      applyBarangayToForm(updated, setters, operatorEmail);
      onBarangayUpdated?.(updated);
      setSaved(true);
      showSuccess('Barangay settings saved.', 'Settings saved');
      setTimeout(() => setSaved(false), 2500);
    } catch (err) {
      setSaveError(err.message ?? 'Save failed.');
      showError(err, 'Could not save barangay settings.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const doReset = async () => {
    ...
    try {
      ...
      const msg = err.message ?? 'Could not reload settings.';
      setLoadError(msg);
      showError(err, msg);
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card title="Barangay Information">
          {barangayLoading && (
            <p className="text-xs text-sk-ink-muted mb-3">Loading barangay settings…</p>
          )}
          <div className="space-y-4">
            <Field
              label="Barangay Name"
            ...
            <CopyableReadOnlyField
              label="Barangay Code"
              value={barangayCode || '—'}
              hint="Share this code with households during registration."
            />
            <Field
              ...
              type="email"
              hint={
                operatorEmail && contactEmail === operatorEmail && !barangayLoading
                  ? 'Using your sign-in email until you save a different contact address.'
                  : undefined
              }
            />
            <Field
...
function Field({ label, value, onChange, type = 'text', min, max, hint }) {
  return (
    <div>
...
function CopyableReadOnlyField({ label, value, hint }) {
  return (
    <div>
      <label className="block text-xs font-bold uppercase tracking-widest text-sk-ink-muted mb-1.5">{label}</label>
      <div className="flex gap-2">
        <div className="flex-1 min-w-0 h-10 rounded-md border border-sk-card-border/40 bg-sk-placeholder/50 px-3 flex items-center">
          <span className="text-sm font-mono font-semibold text-sk-ink truncate">{value}</span>
        </div>
