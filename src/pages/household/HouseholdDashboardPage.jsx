...
export default function HouseholdDashboardPage({ memberName = 'User', householdId = 'HH-01' }) {
  ...
  const savingsTrend = useMemo(
    () => buildSavingsTrend(sim, chartPeriod, shareFactor),
    [sim, chartPeriod, shareFactor],
  );
    ...
    <div className="space-y-5">
        ...
        <div>
          ...
          <p className="text-sm text-sk-ink-muted mt-1">
            ...
            {yourMonthly != null ? (
              <>
                projected share:{' '}
                <strong className="text-emerald-800">{formatCurrency(yourMonthly)}/mo</strong>
              </>
            ) : (
              <>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <Card title="Savings Over Time">
          <p className="text-xs text-sk-ink-muted mb-2">Your household share of projected savings</p>
          ...
          {sim ? (
            <>
              ...
              <p className="text-xs text-emerald-800 mt-2 font-semibold">
                ...
                {householdShare != null && ` · ${householdShare.toFixed(1)}% of community`}
              </p>
            </>
          ) : (
            <p className="text-sm text-sk-ink-muted py-10 text-center">
              No simulation data yet — ask your operator to run a simulation.
            </p>
          )}
        </Card>
          ...
          {sim ? (
            <>
              ...
              <p className="text-xs text-sk-ink-muted mt-2">
                Community peak-hour reduction: {sim.grid_reduction_pct}% less grid use.
              </p>
            </>
          ) : (
